...
public class Player extends MovingObject
{
    ...
    private final int[] SPRITEDELAYS = {30, 8, -1, 8, 3, 3, 3};

    private boolean hasAttack = true;
    private boolean hasDoubleJump = false;
    private boolean hasDash = false;

    private int invulnearableTime = 10;

    private boolean invulnearable = false;
    ...
    public void initPlayer(Vector2 position)
    {
        ...
        collisionBox = new CollisionBox(position,
                new Vector2(tileSize / 3 - 1, tileSize / 2 - 1));
        collisionOffset = new Vector2(tileSize / 2 - 1,
                collisionBox.halfSize.y);
    }
    ...
    private void handleInputs()
    {
        ...
        case IDLE:
            ...
            if (hasJumped && !KeyHandler.isPressed(Keys.JUMP))
                hasJumped = false;
            if (KeyHandler.isPressed(Keys.RIGHT) != KeyHandler
                .isPressed(Keys.LEFT))
            {
                setAnimation(JUMPING);
                break;
            }
            else if (KeyHandler.isPressed(Keys.DOWN)
                    && KeyHandler.isPressed(Keys.JUMP))
            {
                if (isOnPlatform)
                {
                    position.y += 1;
        ...
        case WALKING:
            ...
            if (KeyHandler.isPressed(Keys.DOWN)
                    && KeyHandler.isPressed(Keys.JUMP))
            {
                if (isOnPlatform)
                {
        ...
        case JUMPING:
            ...
            if (isOnGround)
            {
                if (KeyHandler.isPressed(Keys.LEFT) == KeyHandler
    ...
    private void blink()
    {
        //load blink animation after enemy attack
